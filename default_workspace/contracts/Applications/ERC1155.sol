        ...
        if(to.code.length > 0) {
            require(
                IERC1155TokenReceiver(to).onERC1155Received(
                    ...
                    value,
                    data
                ) == IERC1155TokenReceiver.onERC1155Received.selector, "unsafe transfer"
            );
        }
    }
    function safeBatchTransferFrom (
        ...
        require(
            msg.sender == from || isApprovedForAll[from] [msg.sender], " not approved "
        );
        require( to != address(0), "to = 0 address");
        ...
        if (to.code.length > 0) {
            require(
                IERC1155TokenReceiver(to).onERC1155BatchReceived(
                    ...
                    values,
                    data
                ) == IERC1155TokenReceiver.onERC1155BatchReceived.selector, "unsafe transfer"
            );
        }
    }
